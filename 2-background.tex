...
\newcite{5947611} develop a modification to their recurrent neural network architecture for language models. They
factorize the output layer by introducing a hidden layer that assigns a \emph{class} to each word
based on frequency, so that the output layer only evaluates words of the class predicted by the hidden layer.
Although the application is different, our method also involves frequency-based grouping of symbols.\\